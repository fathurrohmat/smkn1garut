...
class Komentar extends CI_Controller{
    ...
    function index(){
        $x['data']=$this->db->query("SELECT komentar.*,tulisan_judul,tulisan_slug FROM komentar JOIN tulisan ON komentar_tulisan_id=tulisan_id ORDER BY komentar_id DESC");
        $this->load->view('admin/v_komentar',$x);
    }

    function publish(){
        $kode = htmlspecialchars($this->uri->segment(4),ENT_QUOTES);
        $this->db->query("UPDATE komentar SET komentar_status='1' WHERE komentar_id='$kode'");
    ...
    function hapus(){
  		...
  		$this->db->delete('komentar', array('komentar_id' => $kode));
