...
class Tulisan extends CI_Controller{
	...
	function index(){
		$x['data']=$this->model_tulisan->get_all_tulisan();
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $gambar=$gbr['file_name'];
													...
													$kategori_id=$this->input->post('xkategori');
													...
													$this->model_tulisan->simpan_tulisan($judul,$isi,$kategori_id,$kategori_nama,$imgslider,$user_id,$user_nama,$gambar,$slug);
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $judul=$this->input->post('xjudul');
													...
													$user_id=$p['pengguna_id'];
													$user_nama=$p['pengguna_nama'];
													$this->model_tulisan->update_tulisan($tulisan_id,$judul,$isi,$kategori_id,$kategori_nama,$imgslider,$user_id,$user_nama,$gambar,$slug);
	                    ...
	                    redirect('admin/pengguna');
									...
									$user_id=$p['pengguna_id'];
									$user_nama=$p['pengguna_nama'];
									$this->model_tulisan->update_tulisan_tanpa_img($tulisan_id,$judul,$isi,$kategori_id,$kategori_nama,$imgslider,$user_id,$user_nama,$slug);
									echo $this->session->set_flashdata('msg','info');
									redirect('admin/tulisan');
	            }
